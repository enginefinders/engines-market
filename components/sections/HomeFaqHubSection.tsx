...
export default function HomeFaqHubSection() {
  ...
  const [manualOpenItemId, setManualOpenItemId] = useState<string | null>(homeFaqItems[0]?.id ?? null);
  ...
  const itemStates = useMemo(
    ...
    [activeCluster, activeBrand, deferredQuery],
  );

  const firstVisibleId = useMemo(
    () => itemStates.find((entry) => entry.visible)?.item.id ?? null,
    [itemStates],
  );
  const visibleCount = useMemo(
    () => itemStates.filter((entry) => entry.visible).length,
    [itemStates],
  );
  const openItemId =
    manualOpenItemId && itemStates.some((entry) => entry.visible && entry.item.id === manualOpenItemId)
      ? manualOpenItemId
      : firstVisibleId;
      ...
      <Container className="max-w-[1200px]">
        <div className="mx-auto mb-6 max-w-[860px] text-center">
          ...
          <div className="relative mx-auto mt-4 max-w-[560px] text-left text-[#94a3b8]">
            ...
            <input
              ...
              placeholder={homeFaqHeader.searchPlaceholder}
        ...
        <div className="grid items-start gap-6 md:grid-cols-[280px_minmax(0,1fr)]">
          <nav
            ...
            role="tablist"
          >
            {homeFaqClusters.map((cluster, index) => {
              ...
              return (
                <button
                  ...
                  className={`relative flex min-h-[84px] flex-col items-start gap-2 rounded-[16px] border border-[#1e3a5f] px-3 py-3 text-left transition md:min-h-[56px] md:flex-row md:items-center md:gap-3 md:rounded-none md:border-x-0 md:border-t-0 md:px-4 md:py-4 ${
                    isActive
                ...
                >
                  ...
                  <span className="flex-1 text-[12px] font-medium leading-[1.35] md:text-[13px]">
                    <span className="hidden md:inline">{cluster.number} </span>
                    {cluster.label}
                  </span>
                ...
                {homeFaqBrands.map((brand) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex min-w-[84px] flex-none flex-col items-center justify-center gap-1 rounded-[14px] border px-3 py-2 text-center transition ${
                        isActive
                    ...
                    >
                      ...
                      />
                      <span className={`text-[11px] font-semibold ${isActive ? "text-[#0d1b2e]" : "text-[#6b7280]"}`}>
                        {brand.label}
                      </span>
                    </button>
                  );
            ...
            <div className="flex flex-col gap-3 border-b border-[#eef2f7] px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
              <p className="text-[12px] font-medium text-[#64748b]">
                {deferredQuery
                  ...
                  : `You are viewing ${activeClusterData?.label ?? ""} - ${activeBrandData?.label ?? ""}`}
              </p>

              <a
                href="#home-hero-reg-form"
                data-quote-context={`FAQ - ${activeBrandData?.label ?? ""} - ${activeClusterData?.label ?? ""}`}
                ...
                return (
                  ...
                  >
                    <button
                      ...
                      onClick={() => setManualOpenItemId((current) => (current === item.id ? null : item.id))}
                    >
                      <span className="text-[15px] font-semibold leading-[1.45]">
                        {highlightText(item.question, deferredQuery)}
                      </span>
